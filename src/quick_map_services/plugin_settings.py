...
class PluginSettings(object):
    ...
    def default_tile_layer_conn_count(cls):
        return cls.get_settings().value(
            "tile_layer/default_connection_count", 4, int
        )

    @classmethod
    def set_default_tile_layer_conn_count(cls, int_val):
        cls.get_settings().setValue(
            "tile_layer/default_connection_count", int_val
        )
    ...
    def set_hide_ds_id_list(cls, ds_id_list):
        cls.get_settings().setValue(
            "hide_ds_id_list_str", ";".join(ds_id_list)
        )

    @classmethod
    def get_hide_ds_id_list(cls):
        return (
            cls.get_settings().value("hide_ds_id_list_str", "", str).split(";")
        )
