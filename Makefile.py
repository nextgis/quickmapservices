...
QGIS_PLUGINS_DIST_DIR = os.path.join(
...
def process_ui():
    ...
    ui_files = [
        f for f in os.listdir(ui_dir) if os.path.splitext(f)[1] == ".ui"
    ]
        ...
        subprocess.call(
            [
                ...
                os.path.join(
                    plugin_dir,
                    "src",
                    "ui_%s.py" % os.path.splitext(ui_file)[0],
                ),
...
def process_qrc():
    print("process qrc ...")
    qrc_files = [
        f for f in os.listdir(plugin_dir) if os.path.splitext(f)[1] == ".qrc"
    ]
    ...
    for qrc_file in qrc_files:
        ...
        subprocess.call(
            [
                ...
                "-o",
                os.path.join(
                    plugin_dir, "%s_rc.py" % os.path.splitext(qrc_file)[0]
                ),
...
def process_ts():
    ...
    i18n_files = [
        f for f in os.listdir(i18n_dir) if os.path.splitext(f)[1] == ".ts"
    ]

    for i18n_file in i18n_files:
        print("Process %s ..." % os.path.join(i18n_dir, i18n_file))
        subprocess.call(["lrelease", os.path.join(i18n_dir, i18n_file)])
...
def make_zip():
    ...
    zipFile = zipfile.ZipFile(
        os.path.join(skript_dir, plugin_name.lower() + ".zip"), "w"
    )
    ...
    for root, dirs, files in os.walk(src_dir):
        ...
        for f in files:
            ...
            zipFile.write(
                filename,
                os.path.join(
                    plugin_name.lower(), os.path.relpath(filename, src_dir)
                ),
            )
    ...
    for root, dirs, files in os.walk(plugin_dir):
        target_files.extend(
            [
                os.path.join(root, f)
                for f in files
                if os.path.splitext(f)[1] in [".ui", ".py"]
            ]
        )
